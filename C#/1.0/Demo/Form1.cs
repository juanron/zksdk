...
namespace Demo
{
    public partial class Form1 : Form
    {
        ...
        {
            int ret = zkfp.ZKFP_ERR_OK;
            if (zkfp.ZKFP_ERR_OK != (ret = fpInstance.OpenDevice(cmbIdx.SelectedIndex)))
            {
                MessageBox.Show("OpenDevice fail, ret = " + ret);
                return;
            }
            bnInit.Enabled = false;
            bnFree.Enabled = true;
            ...
            captureThread.Start();
            bIsTimeToDie = false;
            textRes.Text = "Open succ, device sn=" + fpInstance.devSn;
